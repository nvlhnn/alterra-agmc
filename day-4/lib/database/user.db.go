...
func (qry *UserDatabase) Delete(id uint) error  {
	...
	var user models.User
	err := qry.db.Where("id = ?", id).Unscoped().Delete(&user).Error
	if  err != nil {
		return err
	}
...
func (qry *UserDatabase) VerifyCredential(user *models.User) (error)  {
	err := qry.db.Where("email = ? AND password = ?", user.Email, user.Password).Take(user).Error
	if  err != nil {
		return err
	}
